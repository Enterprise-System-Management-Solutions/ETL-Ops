...
if __name__ == "__main__":
    for node in nodes:
        ...
        try:
            ...
            var_size = check_mountpoint_size(client, "/var")
            log_size = check_mountpoint_size(client, "/var/log")
            if int(opt_size[3][:-1])>60:
                print(f"/opt Size: {opt_size[2]} free, {opt_size[3]} used")
            if int(tmp_size[3][:-1])>60:
                print(f"/tmp Size: {tmp_size[2]} free, {tmp_size[3]} used")
            if int(var_size[3][:-1])>60:
                print(f"/var Size: {var_size[2]} free, {var_size[3]} used")
            if int(log_size[3][:-1])>60:
                print(f"/var/log Size: {log_size[2]} free, {log_size[3]} used")
            client.close()
